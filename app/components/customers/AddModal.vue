...
const schema = z.object({
  ...
  age: z.coerce.number().int('Возраст должен быть целым числом').min(18, 'Возраст должен быть не менее 18'),
  workShift: z.enum(['day', 'night']),
  objectPinned: z.string().min(2, 'Поле закрепленного объекта обязательно'),
  objectPositionsInput: z.string().min(1, 'Добавьте хотя бы одну позицию объекта')
})
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    form.append('objectPinned', event.data.objectPinned.trim())
    form.append('objectPositions', event.data.objectPositionsInput)
...
<template>
  <UModal v-model:open="open" title="Добавить пользователя" description="Добавьте пользователя в базу данных">
    ...
    <template #body>
      ...
      >
        ...
        <UFormField label="Закрепленный объект" name="objectPinned">
          <UInput
            v-model="state.objectPinned"
            class="w-full"
            placeholder="Корпус А"
          />
        </UFormField>

        <UFormField label="Позиции объекта (через запятую)" name="objectPositionsInput">
          <UInput
            v-model="state.objectPositionsInput"
            class="w-full"
            placeholder="Второй этаж, Туалет 1, Туалет 2"
          />
        </UFormField>
